...
export default function TripHighlights() {
  ...
  const categoryLogic = {
    All: (
      <div>
        <div className="space-y-2 bg-transparent rounded-xl mb-6">
          {itineraryDetails.map((item) => (
            <div key={item.day} className="border-none rounded-md">
              ...
              >
                <span>
                  <span className="bg-[#BF500E] rounded-lg p-1">DAY {item.day}</span> - {item.title}
                </span>
        ...
        <div className="mb-6">
          <h2 className="text-xl font-bold mb-4">Ticket Booking</h2>
          <form className="space-y-4">
            ...
            <div>
              ...
              <input type="number" id="tickets" name="tickets" className="w-full px-4 py-2 border rounded-lg" placeholder="Enter number of tickets" />
            </div>
            <button type="submit" className="px-4 py-2 bg-blue-500 text-white rounded-lg">Book Now</button>
          </form>
        </div>
      </div>
    ),
  ...
  return (
    <div className="bg-pink-100 px-6 py-10 max-w-4xl mx-auto">
      ...
      <div className="flex flex-wrap p-2 rounded-xl gap-4 mb-6 bg-[#F4F6F9]">
        {Object.keys(categoryLogic).map((category) => (
          <button
            key={category}
      ...
      {categoryLogic[selectedCategory]}
    </div>
  );
}
